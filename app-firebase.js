                        ...
                        <textarea id="firebaseConfigInput" placeholder='Paste your config here, like:
{
  ...
  "appId": "..."
                                        ...
                                        <div style="display: flex; flex-direction: column; gap: 5px;">
                                            ${index > 0 ? `
                                                <button class="btn btn-secondary btn-small" 
                                                        onclick="event.stopPropagation(); app.moveTruckUp('${truck.id}', '${this.currentStation}')"
                                                        style="padding: 5px 10px; font-size: 14px;">
                                                    ↑
                                                </button>
                                            ` : '<div style="height: 28px;"></div>'}
                                            ${index < stationTrucks.length - 1 ? `
                                                <button class="btn btn-secondary btn-small" 
                                                        onclick="event.stopPropagation(); app.moveTruckDown('${truck.id}', '${this.currentStation}')"
                                                        style="padding: 5px 10px; font-size: 14px;">
                                                    ↓
                                                </button>
                                            ` : '<div style="height: 28px;"></div>'}
    ...
    async moveTruckUp(truckId, station) {
        console.log('Moving truck up:', truckId, 'in station:', station);
        
        // First, ensure all trucks in this station have stationOrder initialized
        this.trucks.forEach((truck, globalIndex) => {
            if (truck.station === station && truck.stationOrder === undefined) {
                truck.stationOrder = globalIndex;
            }
        });
        ...
        const currentIndex = stationTrucks.findIndex(t => t.id === truckId);
        ...
        // Get the actual truck objects from main array
        const currentTruckId = stationTrucks[currentIndex].id;
        const aboveTruckId = stationTrucks[currentIndex - 1].id;
        
        const currentTruck = this.trucks.find(t => t.id === currentTruckId);
        const aboveTruck = this.trucks.find(t => t.id === aboveTruckId);
        
        // Swap their stationOrder values
        const temp = currentTruck.stationOrder;
        currentTruck.stationOrder = aboveTruck.stationOrder;
        aboveTruck.stationOrder = temp;
        
        console.log('Swapped orders:', currentTruck.name, currentTruck.stationOrder, '<->', aboveTruck.name, aboveTruck.stationOrder);
        
        // Save to database
        if (this.db) {
            await this.updateTruckInDB(currentTruck);
            await this.updateTruckInDB(aboveTruck);
        }
        await this.saveData();
    ...
    async moveTruckDown(truckId, station) {
        console.log('Moving truck down:', truckId, 'in station:', station);
        ...
        if (currentIndex < 0 || currentIndex >= stationTrucks.length - 1) {
        ...
        // Get the actual truck objects from main array
        const currentTruckId = stationTrucks[currentIndex].id;
        const belowTruckId = stationTrucks[currentIndex + 1].id;
        
        const currentTruck = this.trucks.find(t => t.id === currentTruckId);
        const belowTruck = this.trucks.find(t => t.id === belowTruckId);
        
        // Swap their stationOrder values
        const temp = currentTruck.stationOrder;
        currentTruck.stationOrder = belowTruck.stationOrder;
        belowTruck.stationOrder = temp;
        
        console.log('Swapped orders:', currentTruck.name, currentTruck.stationOrder, '<->', belowTruck.name, belowTruck.stationOrder);
        
        // Save to database
        if (this.db) {
            await this.updateTruckInDB(currentTruck);
            await this.updateTruckInDB(belowTruck);
        }
        await this.saveData();
