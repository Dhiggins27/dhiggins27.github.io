                        ...
                        <textarea id="firebaseConfigInput" placeholder='Paste your config here, like:
{
  ...
  "appId": "..."
    ...
    renderCheckIn() {
        ...
        const itemFailStatus = new Map(); // itemId -> {status, lastCheckIn, failedCheckIn}
        ...
        for (const checkIn of previousCheckIns) {
            for (const item of checkIn.items) {
                ...
                if (item.type === 'check' && !itemFailStatus.has(item.id)) {
                    if (item.status === 'pass') {
                        // This item passed most recently, so it's been fixed (or never failed)
                        itemFailStatus.set(item.id, {
                            status: 'pass',
                            lastPassDate: checkIn.date
                        });
                    } else if (item.status === 'fail') {
                        // This item failed most recently - need to check if it was fixed later
                        // by looking at older check-ins
                        let wasFixedLater = false;
                        const failDate = checkIn.date;
                        
                        // Check if there's a more recent pass that we haven't seen yet
                        // (shouldn't happen since we're going newest to oldest, but be safe)
                        for (const laterCheckIn of previousCheckIns) {
                            if (new Date(laterCheckIn.date) > new Date(failDate)) {
                                const laterItem = laterCheckIn.items.find(i => i.id === item.id);
                                if (laterItem && laterItem.status === 'pass') {
                                    wasFixedLater = true;
                                    break;
                                }
                            }
                        ...
                        if (!wasFixedLater) {
                            ...
                            // Mark as passed so we don't show it
                            itemFailStatus.set(item.id, {
                                status: 'pass'
                            });
                        }
