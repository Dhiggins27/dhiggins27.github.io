...
//
// TIMEZONE: All dates use local timezone (Casper, Wyoming - Mountain Time)
// Dates are stored as YYYY-MM-DD strings and parsed in local timezone
//
class FireTruckApp {
    constructor() {
                        ...
                        <textarea id="firebaseConfigInput" placeholder='Paste your config here, like:
{
  ...
  "appId": "..."
    ...
    renderHome() {
        // Get today's date in local timezone (Casper, Wyoming)
        const now = new Date();
        ...
        const todayCheckIns = this.checkIns.filter(c => c.date === today);
                ...
                <div class="card">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                        <button class="btn btn-primary" onclick="app.viewStation('Station 7')" 
                                style="padding: 30px; font-size: 18px; font-weight: bold; display: flex; flex-direction: column; align-items: center; gap: 10px;">
                            <img src="fire-station.webp" alt="Fire Station" style="width: 60px; height: auto;">
                            <span>Station 7</span>
    ...
    renderStation() {
        // Get trucks for this station and sort by stationOrder
        const stationTrucks = this.trucks
            .filter(t => t.station === this.currentStation)
            .sort((a, b) => (a.stationOrder || 0) - (b.stationOrder || 0));
        
        // Initialize stationOrder if needed
        stationTrucks.forEach((truck, index) => {
            if (truck.stationOrder === undefined) {
                const actualTruck = this.trucks.find(t => t.id === truck.id);
                actualTruck.stationOrder = index;
            }
        });
                ...
                ` : `
                    <div class="card">
                        ...
                        <div class="truck-list">
                            ${stationTrucks.map((truck, index) => `
                                <div class="truck-item" style="cursor: default;">
                                    <div class="truck-info" onclick="app.quickCheckIn('${truck.id}')" style="cursor: pointer; flex: 1;">
                                        <h3>${truck.name}</h3>
                                        <p>${truck.type}</p>
                                    </div>
                                    <div style="display: flex; gap: 10px; align-items: center;">
                                        <div style="display: flex; align-items: center; gap: 8px;">
                                            <label style="font-size: 14px; color: #666;">Order:</label>
                                            <select id="order-${truck.id}" 
                                                    onchange="app.updateTruckOrder('${truck.id}', this.value)"
                                                    style="padding: 5px 10px; border: 2px solid #ddd; border-radius: 6px; font-size: 14px; cursor: pointer;">
                                                ${Array.from({length: stationTrucks.length}, (_, i) => i + 1).map(num => `
                                                    <option value="${num}" ${num === index + 1 ? 'selected' : ''}>${num}</option>
                                                `).join('')}
                                            </select>
                                        </div>
                                        <span style="font-size: 24px; cursor: pointer;" onclick="app.quickCheckIn('${truck.id}')">→</span>
                                    </div>
                                </div>
                            `).join('')}
                        </div>
    ...
    renderCheckIn() {
        ...
        // Safety check - ensure truck has a checklist array
        if (!truck.checklist) {
            truck.checklist = [];
        }
                ...
                <div class="card">
                    ...
                    <div class="checklist-items" id="checklistItems">
                        ${checkInData.items.map((item, index) => `
                            <div class="checklist-item ${item.status === 'fail' && item.type === 'check' ? 'failed' : ''}" id="item-${item.id}">
            ...
            <div class="container">
                ...
                <div class="card">
                    ${monthCheckIns.map(checkIn => {
                        const failedItems = checkIn.items.filter(i => i.status === 'fail' && i.type === 'check').length;
                        const date = this.parseLocalDate(checkIn.date);
    ...
    renderViewCheckIn() {
        ...
        const failedItems = checkIn.items.filter(i => i.status === 'fail' && i.type === 'check');
        const compressorItems = checkIn.items.filter(i => i.type === 'compressor');
        const yesNoItems = checkIn.items.filter(i => i.type === 'yesno');
        ...
        return `
            ...
            <div class="container">
                ...
                ${yesNoItems.length > 0 ? `
                    <div class="card">
                        <h2>Yes/No Questions (${yesNoItems.length})</h2>
                        ${yesNoItems.map(item => `
                            <div style="background: #f8f9fa; padding: 10px; border-radius: 6px; margin-bottom: 8px; border-left: 3px solid #6c757d;">
                                <p style="color: #333;">${item.status === 'yes' ? '✓ Yes' : '✗ No'} - ${item.name}</p>
    ...
    async updateTruckOrder(truckId, newOrder) {
        newOrder = parseInt(newOrder);
        
        // Get all trucks in the current station
        const stationTrucks = this.trucks
            .filter(t => t.station === this.currentStation)
            .sort((a, b) => (a.stationOrder || 0) - (b.stationOrder || 0));
        
        // Find the truck being moved
        const movingTruckIndex = stationTrucks.findIndex(t => t.id === truckId);
        const movingTruck = this.trucks.find(t => t.id === truckId);
        
        if (movingTruckIndex === -1) return;
        
        const oldOrder = movingTruckIndex + 1; // Convert to 1-based
        
        if (oldOrder === newOrder) return; // No change
        
        // Reassign all stationOrder values
        stationTrucks.forEach((truck, index) => {
            const actualTruck = this.trucks.find(t => t.id === truck.id);
            
            if (truck.id === truckId) {
                // This is the truck being moved
                actualTruck.stationOrder = newOrder - 1; // Convert to 0-based
            } else {
                // Other trucks shift around
                let currentPos = index + 1; // 1-based position
                
                if (oldOrder < newOrder) {
                    // Moving down: shift trucks up between old and new position
                    if (currentPos > oldOrder && currentPos <= newOrder) {
                        actualTruck.stationOrder = currentPos - 2; // Shift up
                    } else {
                        actualTruck.stationOrder = index;
                    }
                } else {
                    // Moving up: shift trucks down between new and old position
                    if (currentPos >= newOrder && currentPos < oldOrder) {
                        actualTruck.stationOrder = currentPos; // Shift down
                    } else {
                        actualTruck.stationOrder = index;
                    }
                }
        ...
        // Save all updated trucks to database
        if (this.db) {
            for (const truck of stationTrucks) {
                const actualTruck = this.trucks.find(t => t.id === truck.id);
                await this.updateTruckInDB(actualTruck);
            }
        }
    ...
    async handleAddTruck(event) {
        ...
        // Get the highest stationOrder for this station to add at the end
        const stationTrucks = this.trucks.filter(t => t.station === station);
        const maxOrder = stationTrucks.length > 0 
            ? Math.max(...stationTrucks.map(t => t.stationOrder || 0))
            : -1;
    ...
    setItemStatus(itemId, status) {
        ...
        if (item.type === 'yesno') {
            ...
            // Yes/No items are informational - they should NOT get the failed/yellow highlight
            ...
            // Do NOT add 'failed' class to yes/no items - they're informational only
            itemEl.classList.remove('failed');
        } else if (item.type === 'compressor') {
            // Compressor items are informational - no pass/fail highlighting
            itemEl.classList.remove('failed');
        } else {
    ...
    async handleSaveCheckIn() {
        ...
        const shiftEl = document.getElementById('checkInShift');
        const personnelEl = document.getElementById('checkInPersonnel');
        const mileageEl = document.getElementById('checkInMileage');
        const fuelLevelEl = document.getElementById('checkInFuel');
        const notesEl = document.getElementById('checkInNotes');
        
        const date = dateEl ? dateEl.value : '';
        const shift = shiftEl ? shiftEl.value : '';
        const personnel = personnelEl ? personnelEl.value : '';
        const mileage = mileageEl ? mileageEl.value : '';
        const fuelLevel = fuelLevelEl ? fuelLevelEl.value : '';
        ...
        try {
            ...
            // Safety check for undefined checklist
            const checklist = this.currentTruck.checklist || [];
    ...
    exportReport(type) {
        ...
        const filtered = this.checkIns.filter(c => {
            const checkInDate = this.parseLocalDate(c.date);
            ...
            truck.checkIns.forEach(checkIn => {
                ...
                html += `
                    <div class="checkin">
                        ...
                        ${compressorItems.length > 0 ? `
                            <div style="margin-top: 10px;"><strong>Compressor Readings:</strong></div>
                            ${compressorItems.map(item => `
                                <div style="background: #e3f2fd; padding: 10px; margin: 5px 0; border-left: 3px solid #2196f3;">
                                    🔧 ${item.name}: ${item.pressure} PSI
                                    ${item.notes ? `<br>Notes: ${item.notes}` : ''}
                                </div>
                            `).join('')}
                        ` : ''}
                        ${yesNoItems.length > 0 ? `
                            <div style="margin-top: 10px;"><strong>Yes/No Questions:</strong></div>
                            ${yesNoItems.map(item => `
                                <div style="background: #f5f5f5; padding: 10px; margin: 5px 0;">
                                    ${item.status === 'yes' ? '✓' : '✗'} ${item.name}: ${item.status === 'yes' ? 'Yes' : 'No'}
                                    ${item.notes ? `<br>Notes: ${item.notes}` : ''}
                                </div>
                            `).join('')}
                        ` : ''}
