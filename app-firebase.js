                        ...
                        <textarea id="firebaseConfigInput" placeholder='Paste your config here, like:
{
  ...
  "appId": "..."
    ...
    renderCheckIn() {
        ...
        // Find all previous check-ins for this truck, sorted by date (most recent first)
        const previousCheckIns = this.checkIns
            .filter(c => c.truckId === truck.id && c.date !== today)
            .sort((a, b) => new Date(b.date) - new Date(a.date));
        
        // Track which items are still failed
        const itemFailStatus = new Map(); // itemId -> {status, lastCheckIn}
        
        // Go through check-ins from most recent to oldest
        for (const checkIn of previousCheckIns) {
            for (const item of checkIn.items) {
                // Only track Pass/Fail items (not yes/no, not compressor)
                if (item.type === 'check' && !itemFailStatus.has(item.id)) {
                    if (item.status === 'fail') {
                        // This item failed and hasn't been fixed yet
                        itemFailStatus.set(item.id, {
                            status: 'fail',
                            checkIn: checkIn,
                            itemData: item
                        });
                    } else if (item.status === 'pass') {
                        // This item passed, so it's been fixed
                        itemFailStatus.set(item.id, {
                            status: 'pass'
                        });
                    }
        ...
        // Build carryover list from items that are still failed
        for (const [itemId, statusInfo] of itemFailStatus) {
            if (statusInfo.status === 'fail') {
                const checklistItem = truck.checklist.find(i => i.id === itemId);
                if (checklistItem) {
                    carryoverItems.push({
                        ...checklistItem,
                        failedDate: statusInfo.checkIn.date,
                        failedBy: statusInfo.checkIn.personnel,
                        failedNotes: statusInfo.itemData.notes
                    });
                }
            }
        }
                ...
                ${carryoverItems.length > 0 ? `
                    <div class="card" style="border: 2px solid #ffc107; background: #fff9e6;">
                        ...
                        ${carryoverItems.map((item, index) => {
                            const failDate = new Date(item.failedDate).toLocaleDateString();
                            return `
...
`;
        ...
        failedItems.forEach(timeline => {
            if (!truckGroups[timeline.truckId]) {
                truckGroups[timeline.truckId] = {
                    name: timeline.truckName,
                    items: []
                };
            }
            truckGroups[timeline.truckId].items.push(timeline);
        });
        
        Object.values(truckGroups).forEach(truck => {
            html += `<div class="truck-section">
                <div class="truck-header">${truck.name} - Item History</div>`;
            
            truck.items.forEach(timeline => {
                html += `
                ...
                timeline.events.forEach(event => {
                    ...
                    // Show original fail info for carryover items
                    let displayDate = eventDate;
                    let displayPerson = event.personnel;
                    if (event.isCarryover && event.originalFailDate) {
                        displayDate = `${eventDate} (originally ${new Date(event.originalFailDate).toLocaleDateString()})`;
                        if (event.originalFailedBy && !isPass) {
                            displayPerson = `${event.personnel} (originally ${event.originalFailedBy})`;
                        }
                    }
                    ...
                    html += `
                        <div class="event ${isPass ? 'event-pass' : 'event-fail'}">
                            <div class="event-header">
                                • ${displayDate} - ${statusLabel} by ${displayPerson}
                            </div>
        ...
        // If no failures, show message
        if (failedItems.length === 0) {
            html += `
                <div style="text-align: center; padding: 40px; color: #28a745; font-size: 18px;">
                    ✓ No failed items in this period - all check-ins passed!
                </div>
            `;
        }
