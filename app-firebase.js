                        ...
                        <textarea id="firebaseConfigInput" placeholder='Paste your config here, like:
{
  ...
  "appId": "..."
    ...
    async moveTruckUp(truckId, station) {
        // Ensure all trucks in this station have stationOrder
        ...
        stationTrucks.forEach((truck, index) => {
            if (truck.stationOrder === undefined) {
                truck.stationOrder = index;
            }
        });
        
        // Sort by stationOrder
        stationTrucks.sort((a, b) => a.stationOrder - b.stationOrder);
        
        const currentIndex = stationTrucks.findIndex(t => t.id === truckId);
        if (currentIndex <= 0) return; // Already at top
        
        // Swap order values with the truck above
        const currentTruck = stationTrucks[currentIndex];
        const aboveTruck = stationTrucks[currentIndex - 1];
        ...
        // Update in main trucks array
        const currentTruckIndex = this.trucks.findIndex(t => t.id === currentTruck.id);
        const aboveTruckIndex = this.trucks.findIndex(t => t.id === aboveTruck.id);
        this.trucks[currentTruckIndex] = currentTruck;
        this.trucks[aboveTruckIndex] = aboveTruck;
    ...
    async moveTruckDown(truckId, station) {
        ...
        const currentIndex = stationTrucks.findIndex(t => t.id === truckId);
        if (currentIndex < 0 || currentIndex >= stationTrucks.length - 1) return; // Already at bottom
        
        // Swap order values with the truck below
        const currentTruck = stationTrucks[currentIndex];
        const belowTruck = stationTrucks[currentIndex + 1];
        ...
        // Update in main trucks array
        const currentTruckIndex = this.trucks.findIndex(t => t.id === currentTruck.id);
        const belowTruckIndex = this.trucks.findIndex(t => t.id === belowTruck.id);
        this.trucks[currentTruckIndex] = currentTruck;
        this.trucks[belowTruckIndex] = belowTruck;
